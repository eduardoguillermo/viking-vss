...
const PANELS=['clientes','alta','detalle','versiones','tipos','backup','presupuestos','stock','catalogo','movimientos','ordenes','config'];
...
function renderConfig(){
  ...
  h += cfgTxt('Zpro Base','cfg-desc-Base', cfg.desc_Base||'Sistema de seguridad basico con control remoto via Telegram.');
  h += cfgTxt('Zpro Energy','cfg-desc-Energy', cfg.desc_Energy||'Sistema con gestion energetica integrada y bateria de respaldo.');
  h += cfgTxt('Zpro Comfort','cfg-desc-Comfort', cfg.desc_Comfort||'Sistema avanzado con automatizacion e integracion Zigbee completa.');
  h += cfgTxt('Zpro Black','cfg-desc-Black', cfg.desc_Black||'Sistema de maxima prestacion con soporte prioritario.');
  el.innerHTML = h;
}

function cfgInp(label, id, val, ph){
  return '<div class="fg" style="margin:0"><label>'+label+'</label>'+
    '<input id="'+id+'" value="'+val.replace(/"/g,"'")+'" placeholder="'+ph+'" oninput="saveConfig()"></div>';
}
