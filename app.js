...
function convertirCliente(id){
  ...
  cerrarModal();
  goTo('alta');
  // Llenar campos con setTimeout para asegurar que el panel esté renderizado
  setTimeout(function(){
    ...
    f('at', p.tel);
    f('aemail', p.email);
    f('aambientes', p.ambientes);
    f('af', today());
    f('av', '');
    var am=document.getElementById('am');
    if(am) am.value=p.modelo||'Base';
    alert('Datos cargados. Completá lote, MAC, PIN y versión instalada.');
  }, 100);
}
...
function generarPDF(id){
  ...
  const body=
    ...
    '<div class="section"><div class="st">Sistema propuesto</div>'+
      ...
      '<div class="mdesc">'+(DB.config&&DB.config['desc_'+p.modelo]?DB.config['desc_'+p.modelo]:{Base:'Sistema de seguridad basico con control remoto via Telegram. Notificaciones en tiempo real y activacion remota.',Energy:'Sistema con gestion energetica integrada, monitoreo de corte de suministro y bateria de respaldo.',Comfort:'Sistema avanzado con automatizacion, integracion Zigbee completa y notificaciones inteligentes.',Black:'Sistema de maxima prestacion con multiples usuarios, historial extendido y soporte prioritario.'}[p.modelo]||'')+'</div>'+
    '</div>'+
...
function renderConfig(){
  ...
  document.getElementById('config-body').innerHTML =
    '<div class="fg2">'+
    '<div class="fg"><label>Nombre de la empresa</label><input id="cfg-empresa" value="'+(cfg.empresa||'')+'" placeholder="Viking Security Systems" oninput="saveConfig()"></div>'+
    '<div class="fg"><label>Email de contacto</label><input id="cfg-email" type="email" value="'+(cfg.email||'')+'" placeholder="contacto@empresa.com" oninput="saveConfig()"></div>'+
    '<div class="fg"><label>Teléfono</label><input id="cfg-tel" value="'+(cfg.tel||'')+'" placeholder="+54 9 261..." oninput="saveConfig()"></div>'+
    '<div class="fg"><label>Dirección</label><input id="cfg-direccion" value="'+(cfg.direccion||'')+'" placeholder="Calle y número, ciudad" oninput="saveConfig()"></div>'+
    '<div class="fg"><label>Sitio web</label><input id="cfg-web" value="'+(cfg.web||'')+'" placeholder="www.empresa.com" oninput="saveConfig()"></div>'+
    '<div class="fg"><label>Firma en emails</label><input id="cfg-firma" value="'+(cfg.firma||cfg.empresa||'')+'" placeholder="Firma para los emails" oninput="saveConfig()"></div>'+
    '</div>'+
    '<hr class="div">'+
    '<div class="sectitle">Descripciones de modelos (aparecen en el PDF)</div>'+
    '<div class="fg"><label>Zpro Base</label><textarea id="cfg-desc-Base" style="min-height:60px" oninput="saveConfig()">'+( cfg.desc_Base||'')+'</textarea></div>'+
    '<div class="fg"><label>Zpro Energy</label><textarea id="cfg-desc-Energy" style="min-height:60px" oninput="saveConfig()">'+( cfg.desc_Energy||'')+'</textarea></div>'+
    '<div class="fg"><label>Zpro Comfort</label><textarea id="cfg-desc-Comfort" style="min-height:60px" oninput="saveConfig()">'+( cfg.desc_Comfort||'')+'</textarea></div>'+
    '<div class="fg"><label>Zpro Black</label><textarea id="cfg-desc-Black" style="min-height:60px" oninput="saveConfig()">'+( cfg.desc_Black||'')+'</textarea></div>';
}
